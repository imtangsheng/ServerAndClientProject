    ...
    , ui(new Ui::CameraWidget)
{
    ...
    deviceType = Camera;
    IWidget::initialize();
    gSouth.RegisterHandler(_module(),this);// 注册当前类的成员函数作为处理器

    //直接函数指针（仅限静态成员函数）
    //
    // 使用std::bind将成员函数绑定到当前对象  需要显式指定占位符 _1
    //gSouth.handlerBinarySession[south::ModuleName::camera] = std::bind(&CameraPlugin::handle_binary_message, this, std::placeholders::_1);
    // 注册当前类的成员函数作为处理器
    gSouth.handlerBinarySession[south::ModuleName::camera] = [this](const QByteArray& bytes) {
        qDebug() <<"gSouth.handlerBinarySession[south::ModuleName::camera] = [this](const QByteArray& bytes"<< bytes.size();
...
{
    gClient.sendTextMessage(Session::RequestString(11,_module(),"test","测试参数"));
}
...
{
    // gClient.sendTextMessage(Session::RequestString(11,_module(),"scan",true));
...
{
    gClient.sendTextMessage(Session::RequestString(11,_module(),"onConfigChanged",QJsonArray()));
}
...
{
    gClient.sendTextMessage(Session::RequestString(11,_module(),"GetUpdateFrameInfo",true));
}
...
{
    //ToDo 显示相机的属性控制界面
    gClient.sendTextMessage(Session::RequestString(11,_module(),"show",true));
}
