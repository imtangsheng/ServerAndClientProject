    ...
    ui(new Ui::MainWindow)
{
    ...
    // 实现一个动画效果
    ui->toolButton_Robot->raise();
    QPropertyAnimation* upAnimation = new QPropertyAnimation(ui->toolButton_Robot, "pos", this);
    upAnimation->setDuration(1500);
    upAnimation->setStartValue(QPoint(ui->toolButton_Robot->x(), ui->toolButton_Robot->y() + 30));
    upAnimation->setEndValue(ui->toolButton_Robot->pos());

    QPropertyAnimation* downAnimation = new QPropertyAnimation(ui->toolButton_Robot, "pos", this);
    downAnimation->setDuration(1500);
    downAnimation->setStartValue(upAnimation->endValue());
    downAnimation->setEndValue(upAnimation->startValue());

    animationGroup = new QSequentialAnimationGroup(this);
    animationGroup->addAnimation(upAnimation);
    animationGroup->addAnimation(downAnimation);
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(event->type() == QEvent::MouseButtonPress){
        ...
        }else if(obj == ui->widget_DataManager){
            onDataManagerClicked();
        }else if(obj == ui->widget_UserDocument){
            onUserDocumentClicked();
        }
    }
...
void MainWindow::onUserDocumentClicked()
{
    static const QString UserDocumentPath = "用户手册.pdf";
    QStringList searchPaths;
    searchPaths << QCoreApplication::applicationDirPath()  // exe目录
                << QCoreApplication::applicationDirPath() + "/doc"  // exe下的doc目录
                << QCoreApplication::applicationDirPath() + "/docs" // exe下的docs目录
                << QCoreApplication::applicationDirPath() + "/../doc"; // 上级目录的doc
    QString pdfPath;
    for (const QString& path : std::as_const(searchPaths)){// 在所有可能的路径中查找PDF文件
        QString testPath = path + QDir::separator() + UserDocumentPath;
        if (QFile::exists(testPath)){
            pdfPath = testPath;break;
        }
    }
    if (pdfPath.isEmpty()){// 如果找不到文件
        show_message(tr("The user manual file does not exist, please contact the relevant personnel"),LogLevel::Error);
        return;
    }
    if(QDesktopServices::openUrl(QUrl::fromLocalFile(pdfPath))){// 尝试打开文件
        show_message(tr("The user manual cannot be opened, please make sure the system has selected the default PDF opening software"),LogLevel::Error);
        return;
    }
}

void MainWindow::onDeviceManagerClicked()
{
    qDebug() << "MainWindow::onDeviceManagerClicked()";
}

void MainWindow::onDataManagerClicked()
{
    qDebug() << "MainWindow::onDataManagerClicked()";
}
...
{
    ui->pushButton_Network_State->setText(msg);
    ui->pushButton_Network_State->setEnabled(!isConnecting);
    ui->toolButton_devices_powered_off->setEnabled(isConnected);
    if(isConnected){
        animationGroup->start();
    }else{
        animationGroup->stop();
...
void MainWindow::on_toolButton_Robot_clicked()
{
    animationGroup->start();
}
