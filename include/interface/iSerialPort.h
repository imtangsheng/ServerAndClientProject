...
namespace serial {
    ...
    Q_NAMESPACE
        ...
        SERIAL_CLOVER = 0x02,//相机设备
        SERIAL_ALL = SERIAL_CAR | SERIAL_CLOVER
    };
    Q_ENUM_NS(SerialDeviceType)
        // 这个枚举必须在某处通过Q_DECLARE_METATYPE注册
        //Q_DECLARE_METATYPE(SerialDeviceType)
        inline static quint8 g_serial_device_type = SERIAL_ALL;//默认是小车和相机都支持
...
inline QString FunctionCodeToString(FunctionCodeType code) {
    ...
    return QString("funtion0x") + QString::number(code, 16).rightJustified(2, '0').toUpper();
}
// 帧格式定义
...
/*定义的数据,用于缓存数据*/
static inline ScannerTimeInfo scanner_time_info_;
static inline qint64 scanner_time_count_{ 0 };//计数

static inline qint64 mileage_count_{ 0 };//里程计数

inline static constexpr int kMileageUpdateInterval = 20;//界面中的更新推送次数
inline double speed_multiplier = 1.0;//速度倍率

inline const QJsonObject SPEED_MULTIPLIER{
...
struct CloverTriggerInfo
{
    ...
    quint8 position;//    旋转机构位置，取值 1 - 15
};
