...
//定义快捷方式
#define gSouth south::South::instance()
#define sigSouthSend south::South::instance().sigSend

namespace south {

class SHAREDLIB_EXPORT South : public QObject
{
	Q_OBJECT
public:
	static South& instance() {//使用引用,返回其静态变量,不进行拷贝数据
...
private:
	...
	South() {
		this->setObjectName("South");
		initialize();
	}
	void initialize() {
		workerThread = new QThread();
		workerThread->setObjectName("South");
	...
	South(const South&) = delete;
	South& operator=(const South&) = delete;
signals:
