...
typedef enum dvpStreamFormat
{
	...
	/**黑白彩色2种格式组合方式，同时输出BGR24和MONO8*/
    S_BGR24_MONO8 = 90,	
	
	/**黑白彩色2种格式组合方式，同时输出RGB24和MONO8*/
	S_RGB24_MONO8 = 91,	
...
typedef enum dvpXMLVisibility
{	
	Visibility_TYPE_BEGINNER	= 0,
	Visibility_TYPE_EXPERT	= 1,
	Visibility_TYPE_GURU	= 2,
	Visibility_TYPE_INVISIBLE	= 3,
	Visibility_TYPE_UNDEFINED	= 99

} dvpXMLVisibility;

typedef struct dvpXmlNodeFeature
{
    enum dvpXMLInterfaceType enType;
    enum dvpXMLVisibility enVisivility;
    dvpString256 strDescription;
    dvpString64 strDisplayName;
    dvpString64 strName;
    dvpString256 strToolTip;
...
*@param[out] pValue    当前值
*@param[out] pIntDescr int属性描述信息结构体
*/
...
*@param[in]  strKey        属性键值，如获取宽度信息则为“Width”
*@param[out] pValue        当前值
*@param[out] pUint64Descr  Uint属性描述信息结构体
*/
...
*@param[in]  strKey         属性键值，如获取宽度信息则为“Width”
*@param[out] pValue         当前值
*@param[out] pUintU32Descr  Uint32属性描述信息结构体
*/
...
*@param[out] pValue          返回当前的枚举值
*@param[out] SupportValue    返回给调用者支持的所有枚举值,保留可传NULL
*@param[out] pSupportNum     有效的枚举值个数,保留可传NULL
*Example:
unsigned int nCurEnumValue;
dvpGetEnumValue(m_hHandle,"TargetFormat",&nCurEnumValue,NULL,NULL);
若相机能支持的所有目标图像格式列表为{S_RAW8=0,S_BGR24=10,S_BGR32=11},当前格式为S_BGR24的话，nCurEnumValue返回10
@note:接口保留，不建议使用
*/
...
*@param[in] handle 设备句柄
*@param[in] strKey 属性键值，如获取像素格式信息则为“PixelFormat”
*@param[out] pCurSel   返回当前的序号值
*@param[out] pValue    返回当前的枚举值
*@param[out] strValue  返回当前的枚举字符串
*/
...
*@param[in] handle 设备句柄
*@param[in] strKey 属性键值，如获取像素格式信息则为“PixelFormat”
*@param[out] pSupportValues 返回给调用者支持的所有枚举值,不允许为NULL
*@param[in,out] pSupportNum 输入为pSupportValues数组的大小,返回实际的枚举值个数,不允许为NULL
*/
...
/**
*@brief 通过枚举值下标获取Enum属性值和名称
*@param[in] handle 设备句柄
*@param[in] strKey 属性键值，如获取像素格式信息则为“PixelFormat”
*@param[in] uIndex 枚举值序号
*@param[out] pEnumDescr 对应的枚举值属性结构
*/
...
/**
*@brief 通过枚举值下标获取Enum属性值和名称(同步获取方式接口)
*@param[in] handle 设备句柄
*@param[in] strKey 属性键值，如获取像素格式信息则为“PixelFormat”
*@param[in] uIndex 枚举值序号
*@param[out] pEnumDescr 对应的枚举值属性结构
...
*@param[in] strKey 属性键值
*@param[out] pValue 当前值
*@param[out] pDoubleDescr 对应的枚举值属性结构
*/
...
*@param[in] handle 相机句柄
*@param[in] key    属性字符串
*@param[in] value   设置的属性值
*@warning 此函数为内部使用的函数，参数细节暂未公开，不建议使用
*/
dvp2api dvpStatus dvpSetConfigString(dvpHandle handle, dvpStr key, dvpStr value);

/**
*@brief 获取相机所有属性节点的详细信息
*@param[in] handle 相机句柄
*@param[in,out] pNodeFeatureList  属性节点详细信息结构体
*@param[in,out] nNodeNum          属性节点个数
*@warning 此函数为内部使用的函数，参数细节暂未公开，不建议使用
*/
...
public:
	...
	*/
	...
	{
	     return dvpSetIntValue(handle,strKey,iValue);
	}
	...
	*/
	...
	{
	    return dvpSetUint64Value(handle,strKey,uValue);
	}
	...
	*/
	...
	{
	    return dvpSetEnumValue(handle,strKey,iValue);
	}
	...
	*/
	...
	{
	    return dvpSetFloatValue(handle,strKey,fValue);
	}
	...
	*/
	...
	{
	    return dvpSetDoubleValue(handle,strKey,lfValue);
	}
	...
	*/
	...
	{
	    return dvpSetBoolValue(handle, strKey, bValue);
	}
	...
	*/
	...
	{
	    return dvpSetStringValue(handle,strKey,strValue);
	}
	...
	*/
	...
	{
	     return dvpGetIntValue(handle,strKey,pValue,pIntDescr);
	}
	...
	*/
	...
	{
	     return dvpGetUint64Value(handle,strKey,pValue,pUint64Descr);
	}
	...
	*/
	...
	{
	    return dvpGetEnumValueEx(handle,strKey,pCurSel,pValue,strValue);
	}
	...
	*/
	...
	{
	    return dvpGetFloatValue(handle,strKey,pValue,pFloatDescr);
	}
	...
	*/
	...
	{
	    return dvpGetDoubleValue(handle,strKey,pValue,pDoubleDescr);
	}
	...
	*/
	...
	{
	    return dvpGetBoolValue(handle,strKey,pValue);
	}
	...
	*/
	...
	{
	    return dvpGetStringValue(handle,strKey,strValue,iValueSize);
	}

	/**
	*@brief 刷新已连接的相机列表，并获得相机的数量
	*@param[out] pCount 相机总数
	*@note 参考dvpRefresh api函数说明
	*/
	static dvpStatus dvpRefresh(dvpUint32 *pCount)
	{
	    return dvpRefresh(pCount);
	}
	...
	*@param[in] mode 打开方式
	*@param[out] pHandle 相机句柄
	*@note 参考dvpOpen api函数说明
	*/
	static dvpStatus dvpOpen(dvpUint32 index, dvpOpenMode mode, dvpHandle *pHandle)
	{
	    return dvpOpen(index,mode,pHandle);
	}

	/**
	*@brief 启动视频流，分配必要的资源
	*@param[in] handle 相机句柄
	*@note 参考dvpStart api函数说明
	*/
	...
	*@brief 停止视频流，释放相应的资源
	*@param[in] handle 相机句柄
	*@note 参考dvpStop api函数说明
	*/
	static dvpStatus dvpStop(dvpHandle handle)
	{
	   return  dvpStop(handle);
	}
	...
	*@param[out] pBuffer 图像数据的内存首地址 @warning 切勿手动释放
	*@param[in] timeout 超时时间（毫秒）
	*@note 参考dvpGetFrame api函数说明
	*/
	static dvpStatus dvpGetFrame(dvpHandle handle, dvpFrame *pFrame, void **pBuffer, dvpUint32 timeout)
	{
	   return  dvpGetFrame(handle,pFrame,pBuffer,timeout);
	}

	/**
	*@brief 关闭相机，并进入待机状态
	*@param[in] handle 相机句柄
	*@note 参考dvpClose api函数说明
	*/
