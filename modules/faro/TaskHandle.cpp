...
{
	if (hasClinometer) {
		return expandPointcloud(part.points, part.mileageWithTime, part.clinometerWithTime,
			part.flag, part.direction, part.start_mileage, part.end_mileage);
	}
	else
	{
		return expandPointcloud(part.points, part.mileageWithTime, part.flag, part.start_mileage, part.end_mileage);
	}
}
	...
	Speed flag, double& start_mileage, double& end_mileage)
{
	...
	start_mileage = pointcloudRes[0][0].y;
	end_mileage = pointcloudRes.back().back().y;
...
void save_baseinfo_file(const std::string& taskPath, const std::string& targetName, const std::vector<MileageEllipse>& lme, const std::vector<std::vector<PointCloud>>& pointcloudResTemp)
{
...
static void saveImage(const QString& path,Image &img) {
	...
	//	{
	//		grayImage.at<uchar>(i, j) = img.data[i][j];
	//	}
	//}
...
static void saveImage(const std::vector<std::vector<unsigned char>>& color,
...
bool WritePointCloudImage(TaskFaroPart& part, const QString& grayImagePath, const QString& depthImagePath, int merge_type)
{
	...
	//return generateImages(lme, part.points,
	//	grayImagePath, depthImagePath, part.start_mileage, part.end_mileage, part.resolving, part.diameter / 2);

	
	//if (merge_type == 1) holeDepthLimit= 0.19;
	//else if (merge_type == 0) holeDepthLimit = 0.09;
	//return map.GenerateDepthAndGrayImage(lme, part.points,grayImagePath, depthImagePath, part.start_mileage, part.end_mileage, part.resolving, part.diameter / 2);
